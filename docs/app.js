...
async function uploadToWigle() {
  ...
  try {
    ...
    // "Failed to fetch" here almost always means the HOSTING page blocks outbound
    // requests via a Content-Security-Policy (Neocities sets connect-src 'self'),
    // so the browser refuses the call to WiGLE before it leaves. Combining still
    // worked — guide the user to the reliable download → wigle.net/upload path.
    status.innerHTML =
      `<div class="err"><b>This site can't upload directly to WiGLE.</b> Your web host blocks ` +
      `outbound API calls (a security policy called CSP — common on Neocities and other static hosts). ` +
      `Your combined file is ready, though:</div>` +
      `<div class="fallback-actions">` +
        `<button type="button" id="dlFallback" class="btn">⬇ Download combined CSV</button>` +
        `<a class="btn ghost" href="https://wigle.net/upload" target="_blank" rel="noopener">Open wigle.net/upload ↗</a>` +
      `</div>` +
      `<div class="muted small">Then drag the downloaded file onto WiGLE's upload page. ` +
      `(One-click upload works on a self-hosted copy that doesn't set CSP.)</div>`;
    const b = document.getElementById('dlFallback'); if (b) b.addEventListener('click', download);
  } finally {
